...
def calibrate_daily_quote_meta():
    ...
    for i, stock_item in enumerate(stock_list):
        latest_quote = (
            StockDailyQuote.objects(code=stock_item.code).order_by("-date").first()
        )
        # curr_daily_quote_meta = trading_day_helper.read_freshness_meta(stock_item, 'daily_quote')
        curr_daily_quote_meta = freshness_meta_helper.read_freshness_meta(
        ...
        if latest_quote:
            ...
            if latest_quote_date != curr_daily_quote_meta:
                ...
                counter_flag = "GOOD"
                prog_bar_msg = (
                    f"{stock_item.code} - {stock_item.name} freshness meta check OK"
                )
        else:
