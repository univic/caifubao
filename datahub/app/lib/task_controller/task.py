...
class DatahubTask(object):
    def __init__(self, runner_name="General", task_obj=DatahubTaskDoc):
        ...
        self.task_scan_interval = app_config.DATAHUB_TASK_SCAN_INTERVAL
        self.continue_scan = True

    def dispatch(self):
        logger.info(
            f"{self.runner_name} task worker - Running in process {os.getpid()}, "
        ...
        while self.continue_scan:
            ...
            if self.task_list_length > 0:
                logger.info(
                    f"{self.runner_name} task worker - found {self.task_list_length} task(s)"
                )
                ...
                logger.info(
                    f"{self.runner_name} task worker - no available task，waiting for next scan"
                )
            logger.info(
                f"{self.runner_name} task worker - task scan completed, "
                f"next scan in {self.task_scan_interval} minutes"
            )
            self.after_dispatch()
            mongo_watcher.disconnect_from_db()
            time_to_wait = self.task_scan_interval * 60
            time.sleep(time_to_wait)

    def get_task_list(self):
        # get task list and calculate list length
        self.task_list = self.task_obj.objects(
            status="CRTD"
    ...
    def exec_task_list(self):
        ...
        logger.info(
            f"{self.runner_name} task worker - Processed {self.task_list_length} tasks, "
    ...
    def create_task(
        ...
        **extra_kw,
        ...
        if check_task_uniqueness(new_task):
            new_task.save()
            logger.debug(
                f"{self.runner_name} task worker -   ask {new_task.name} created"
            )
        else:
            logger.debug(
                f"{self.runner_name} task worker - Found duplicate data retrieve task {new_task.name}"
            )
...
class AkshareDatahubTask(DatahubTask):
    ...
    def get_task_list(self):
        # Slice here to limit task number
        self.task_list = self.task_obj.objects(
            _cls="DatahubTaskDoc", status="CRTD", callback_interface="akshare"
        )[:]
        return self.task_list
...
class BaostockDatahubTask(DatahubTask):
    ...
    def get_task_list(self):
        # Slice here to limit task number
        self.task_list = self.task_obj.objects(
            _cls="DatahubTaskDoc", status="CRTD", callback_interface="baostock"
        )[:]
        return self.task_list
...
class ScheduledDatahubTask(DatahubTask):
    ...
    def get_task_list(self):
        self.task_list = self.task_obj.objects(status="CRTD").order_by(
            "-scheduled_time"
        )  # Slice here to limit task number
        return self.task_list
    ...
    def exec_task_list(self):
        if self.task_list:
            ...
            counter = {"COMP": 0, "WARN": 0, "FAIL": 0, "SKIP": 0, "ERR": 0}
            for i, item in enumerate(self.task_list):
                scheduled_run_timestamp = datetime.datetime.timestamp(
                    item.scheduled_process_time
                )
                time_diff = scheduled_run_timestamp - datetime.datetime.timestamp(
                ...
                if exec_flag:
                    logger.info(
                        f"{self.runner_name} task worker - Will run task {item.name} in {int(next_run)} seconds"
                    )
                    time.sleep(next_run)
                    logger.info(
                        f"{self.runner_name} task worker - Running task {item.name}"
                    )
                    result = self.exec_task(item)
                    if result["code"] == "GOOD":
                        counter["COMP"] += 1
                        logger.info(
                            f"{self.runner_name} task worker - Successfully processed task {item.name}"
                        )
                    elif result["code"] == "WARN":
                        counter["WARN"] += 1
                        logger.info(
                            f"{self.runner_name} task worker - Task {item.name} completed with warning message"
                        )
                    elif result["code"] == "ERR":
                        counter["ERR"] += 1
                        logger.info(
                            f"{self.runner_name} task worker - Error when processing task {item.name}"
                        )
                    else:
                        counter["FAIL"] += 1
                        logger.info(
                            f"{self.runner_name} task worker - task {item.name} has failed"
                        )
            logger.info(
                f"{self.runner_name} task worker - Task execution completed, "
            ...
            logger.info(
                f"{self.runner_name} task worker - No scheduled task was found, "
    ...
    def handle_repeat_task(self, item):
        if item.repeat:
            ...
            kw_dict = item.kwargs
            self.create_task(
                name=trading_day_helper.update_title_date_str(item.name, next_run_time),
