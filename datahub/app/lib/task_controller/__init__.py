...
class Queue(object):
    def __init__(self, name: str, attributes: dict = None):
        ...
        self.task_exec_interval: float = app_config.TASK_CONTROLLER_TASK_EXEC_INTERVAL
        self.task_scan_interval = app_config.TASK_CONTROLLER_TASK_SCAN_INTERVAL
        logger.info(
            f"TaskController - Creating task queue: {self.name}, process PID {os.getpid()}, "
    ...
    def dispatch(self, task_queues=None):
        try:
            ...
            logger.info(
                f"TaskController - Queue {self.name} dispatched, process PID {os.getpid()}"
            )
            continue_flag = True
            while continue_flag:
                self.consume_queue()
    ...
    def consume_queue(self):
        self.queue = list(
            Task.objects(status="CRTD", queue=self.name).order_by("-scheduled_time")
        )
        ...
        while queue_length > 0:
            logger.info(
                f"TaskController - Queue {self.name} - Found {queue_length} tasks, running"
            )
            self.exec_task(self.queue[0])
            self.queue.pop(0)
            queue_length = len(self.queue)
    ...
    def exec_task(self, task):
        obj = getattr(
            import_module(f"app.lib.{task.callback_package}.{task.callback_module}"),
        ...
        if result and result["code"]:
            ...
            task.save()
            logger.info(
                f"TaskController - Queue {self.name} - Task '{task.name}' completed successfully"
            )
        else:
            logger.error(
                f"TaskController - Queue {self.name} - Task '{task.name}' did not return valid result info\n"
    ...
    def handle_repeat_task(self, task):
        if task.repeat_duration:
            if task.repeat_duration == "T-DAY":
                ...
                next_run_time += datetime.timedelta(
                    hours=curr_run_time.hour,
            ...
            task_controller.create_task(
                name=trading_day_helper.update_title_date_str(task.name, next_run_time),
...
class TaskQueueController(object):
    ...
    def __init__(self):
        ...
        self.queue_num: int = app_config.TASK_CONTROLLER_DEFAULT_TASK_QUEUE_NUM
        self.max_queue_num: int = app_config.TASK_CONTROLLER_MAX_TASK_QUEUE_NUM
    ...
    def initialize(self):
        ...
        :return:
        """
        logger.info("TaskController - TaskQueueController is initializing")
        default_queue_attrs = {"module": "datahub"}
        if self.use_multi_processing:
            self.process_pool = Pool(4)
            self.task_queues = Manager().dict()
            logger.info(
                f"TaskController - Process pool ready, parent process PID {os.getpid()}"
            )
        else:
            pass
    ...
    def setup_queue(self, name, attributes: dict = None):
        ...
        if self.use_multi_processing:
            ...
            if use_new_process:
                with LOCK:
                    logger.info("TaskQueueController - Setting up queue in new process")
                    self.task_queues[name] = queue
                    self.process_pool.apply_async(
                        queue.dispatch, error_callback=self.err_callback
                    )
            else:
                logger.info("TaskQueueController - Setting up queue in new thread")
    ...
    def distribute_tasks_to_queue(self, task_list: list, find_queue_by):
        ...
        if is_subprocess:
            ...
            for task in task_list:
                ...
                if (
                    hasattr(task, find_queue_by)
                    and getattr(task, find_queue_by) in self.task_queues.keys()
                ):
...
class TaskController(object):
    def __init__(self):
        ...
        logger.info("TaskController is initializing")
        logger.info(f"TaskController - process PID {os.getpid()}")

    @staticmethod
    def check_historical_tasks():
        logger.info("TaskController - Checking historical tasks")
        # Check obsolete tasks
        task_list = Task.objects(
            status="CRTD", valid_before__lte=datetime.datetime.now()
        )
        task_list_len = task_list.count()
        logger.info(
            f"TaskController - Found {task_list_len} obsolete tasks, all of those tasks will be deactivated"
        )
        for task in task_list:
            task.status = "ABORT"
    ...
    def dispatch(self):
        ...
        while self.continue_scan:
            logger.info(
                f"TaskController - Scanning tasks by interval {self.task_scan_interval} seconds"
            )
            self.get_task_list()
            ...
            if self.task_list_length > 0:
                ...
                logger.info(
                    f"TaskController - Added {self.task_list_length} task(s) to the queue, "
                    f"next scan in {self.task_scan_interval} minutes"
                )
            else:
                logger.info(
                    f"TaskController - No new task，next scan in {self.task_scan_interval} seconds"
                )
