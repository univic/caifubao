...
class StockDataCapabilities(EmbeddedDocument):
    daily_quote = BooleanField(default=True)
    fq_factor = BooleanField(default=True)
    ma_factor = BooleanField(default=True)
...
class BasicStock(Document):
    ...
    active_status = IntField(default=0)  # 2 - 退市  1 - 不活跃  0 - 正常
    data_capabilities = EmbeddedDocumentField(
        StockDataCapabilities, default=StockDataCapabilities
    )
...
class FinancialStatus(EmbeddedDocument):
    ...
    earnings_per_share_after_deducting_non_recurring_profits_and_losses = (
        FloatField()
    )  # 扣除非经常性损益后的每股收益(元)
    ...
    net_profit_after_deducting_non_recurring_profit_and_loss = (
        FloatField()
    )  # 扣除非经常性损益后的净利润(元)
    ...
    ratio_of_long_term_assets_to_long_term_funds = (
        FloatField()
    )  # 长期资产与长期资金比率(%)
    ...
    ratio_of_net_operating_cash_flow_to_sales_revenue = (
        FloatField()
    )  # 经营现金净流量对销售收入比率(%)
    return_on_operating_cash_flow_of_assets = (
        FloatField()
    )  # 资产的经营现金流量回报率(%)
    ratio_of_net_operating_cash_flow_to_net_profit = (
        FloatField()
    )  # 经营现金净流量与净利润的比率(%)
    ratio_of_net_operating_cash_flow_to_liabilities = (
        FloatField()
    )  # 经营现金净流量对负债比率(%)
