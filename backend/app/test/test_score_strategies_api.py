...
class FakeQuerySet:
    ...
    def order_by(self, *fields):
        ...
        for field in reversed(fields):
            ...
            rows = sorted(
                rows, key=lambda row: getattr(row, name, None) or 0, reverse=reverse
            )
...
def _mk_pred(
    ...
    status="VERIFIED",
    verification=None,
    model_version="score_v2_202605",
):
    if date is None:
        date = datetime.datetime(2025, 6, 1)
    if verification is None:
        verification = {
            "return_at_target": 0.035,
            "max_return": 0.08,
            "min_return": -0.02,
            "max_drawdown": -0.02,
            "hit_target": True,
...
def test_backtest_computes_metrics(client, monkeypatch):
    p1 = _mk_pred(
        ...
        score=85.0,
        verification={"return_at_target": 0.05, "max_return": 0.08, "hit_target": True},
    )
    p2 = _mk_pred(
        "sz000001",
        "平安银行",
...
def test_confidence_returns_data(client, monkeypatch):
    predicted = _mk_pred(
        "sh600519", "茅台", datetime.datetime(2025, 6, 1), score=85.0, status="PENDING"
    )
    verified = [
        _mk_pred(
            "sh600519",
    ...
    resp = client.get(
        "/api/score-strategies/confidence?stock_code=sh600519&date=2025-06-01&horizon=5"
    )
    assert resp.status_code == 200
    body = resp.get_json()
    assert body["success"] is True
...
def test_confidence_unsupported_horizon_400(client):
    resp = client.get(
        "/api/score-strategies/confidence"
...
def test_confidence_not_found_404(client, monkeypatch):
    ...
    resp = client.get(
        "/api/score-strategies/confidence?stock_code=sh000001&date=2020-01-01&horizon=5"
    )
    assert resp.status_code == 404
