...
import traceback
import pandas as pd
from mongoengine import BulkWriteError, NotUniqueError
...
class GeneralWorker(object):
    ...
    def run_processor(self, processor_dict):
        ...
        process_handler_func = getattr(
            self.processor_instance, processor_dict["handler"]
        )
        # logger.info(f'Doing factor analysis {processor_name} for {self.stock_obj.code} - {self.stock_obj.name}')
        result_dict = process_handler_func()
        self.counter_dict[result_dict["code"]] += 1
        self.result_list.append((processor_dict, result_dict))
...
class GeneralProcessor(object):
    ...
    def __init__(
        ...
        self.exec_result_dict: dict = {"code": "FINI", "msg": ""}
        self.run_msg = ""
        self.freshness_meta_update_flag = False

    def run(self):
        self.run_msg = (
            f"Running {self.meta_type} processor {self.meta_name} "
            f"for {self.stock_obj.code}-{self.stock_obj.name}"
        )
        logger.debug(self.run_msg)
        # prepare the input
    ...
    def check_prerequisite(self):
        pass
        # if not self.input_df or self.input_df.empty:
    ...
    def determine_exec_range(self):
        ...
        self.freshness_meta_dt = freshness_meta_helper.read_freshness_meta(
        ...
        if (
            self.freshness_meta_dt
            == self.scenario.current_datetime_prev_complete_trading_day
        ):
            ...
            # if metadata time is behind current time, do partial analysis
                ...
                if self.freshness_meta_dt in self.input_df.index:
                    # TODO: ADD EXISTENCE CHECK HERE
                    head_index = (
                        self.input_df.index.get_loc(self.freshness_meta_dt)
                    ...
                    logger.warning(
                        f"{self.stock_obj.code}-{self.stock_obj.name} {self.meta_type}-{self.meta_name}"
                        f" Failed to find index by {self.freshness_meta_dt},"
                        f" using full df instead."
                    )
                    self.process_df = self.input_df
            else:
                logger.error(
                    "Unidentified processing circumstance: most recent meta datetime greater than current time"
                )

    def prepare_output(self):
        if self.freshness_meta_dt:
            if self.freshness_meta_dt in self.process_df.index:
                head_index = self.process_df.index.get_loc(self.freshness_meta_dt)
                output_start_index = head_index + 1
                self.output_start_date = self.input_df.index[output_start_index]
                self.output_df = self.process_df.iloc[output_start_index:][:]
                logger.debug(
                    f"{self.stock_obj.code}-{self.stock_obj.name} {self.meta_type}-{self.meta_name}"
                    f" sliced output df from {self.freshness_meta_dt}"
                )
            else:
                logger.warning(
                    f"{self.stock_obj.code}-{self.stock_obj.name} {self.meta_type}-{self.meta_name}"
                    f" Failed to slice output df by {self.freshness_meta_dt},"
                    f" using full df instead."
                )
    ...
    def perform_db_upsert(self):
        if self.bulk_insert_list:
            try:
                # try bulk insert
                self.db_document_object.objects.insert(
                    self.bulk_insert_list, load_bulk=False
                )
                self.freshness_meta_update_flag = True
            # update database
            except KeyError as e:
                logger.warning(
                    f"Key error when trying to bulk insert. traceback info: {traceback.format_exception(e)}"
                )
            except BulkWriteError as e:
                if len(e.args) and "E11000 duplicate key" in e.args[0]:
                    msg = f"Duplicate key error when trying bulk insert, Trying to upsert one by one. {traceback.format_exception(e)}"
                    for item in self.bulk_insert_list:
                        try:
                            item.save()
                            logger.info(f"Item saved successfully: {item}")
                        except NotUniqueError:
                            logger.warning(f"Item not unique: {item}")
                else:
                    msg = f"Error when trying bulk insert. {traceback.format_exception(e)}"
                logger.warning(msg)
                self.set_exec_result_state("WARN", msg)
        else:
            logger.error(
                f"{self.stock_obj.code}-{self.stock_obj.name} {self.meta_type}-{self.meta_name} "
                f"Empty output df"
    ...
    def update_freshness_meta(self):
        if self.freshness_meta_update_flag:
            latest_stock_quote_date = max(self.output_df.index)
            latest_date = self.scenario.current_datetime_prev_complete_trading_day
            # 取最早的日期
            earlier_date = min(latest_stock_quote_date, latest_date)
            ...
            logger.warning(
                f"{self.stock_obj.code}-{self.stock_obj.name} {self.meta_type}-{self.meta_name}"
                f" Freshness meta update was prohibited due to "
                f" abnormal processor execution."
            )
