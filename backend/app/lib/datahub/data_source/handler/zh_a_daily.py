...
def get_a_stock_trade_date_hist():
    remote_data = interface.akshare_interface.get_trade_date_hist()
    # convert to datetime
...
def get_zh_a_stock_index_spot():
    ...
    raw_df = interface.akshare_interface.zh_stock_index_spot()
...
def get_zh_a_stock_spot():
    ...
    try:
        raw_df = interface.akshare_interface.stock_zh_a_spot_em()
    except ConnectionError as e:
        logger.warning(
            f"ConnectionError occurred when calling stock_zh_a_spot_em(): {e}, falling back to stock_zh_a_spot()"
        )
        try:
            raw_df = interface.akshare_interface.stock_zh_a_spot()
        except ConnectionError as e:
            logger.error(
...
def get_zh_a_index_hist_daily_quote(code, start_date=None, incremental=True):
    raw_df = interface.akshare_interface.stock_zh_index_daily(code)
    raw_df.fillna(0, inplace=True)
