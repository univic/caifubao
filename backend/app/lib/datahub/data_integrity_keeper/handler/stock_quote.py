...
class DataIntegrityWatcher(object):
    ...
    def before_check(self):
        ...
        self.most_recent_quote_date = trading_day_helper.determine_closest_trading_date(
            self.trade_calendar
        )
    ...
    def check_data_freshness_meta(self, stock_item):
        check_flag = "GOOD"
        daily_quote_freshness_meta = freshness_meta_helper.read_freshness_meta(
        ...
        if daily_quote_freshness_meta != latest_quote_date:
            ...
            self.abnormal_list.append(
                f"{stock_item.code} - {stock_item.name}: Data freshness meta does not match with quote data"
            )
        return check_flag
    ...
    def check_quote_data_continuity(self, item):
        check_flag = "GOOD"
        quote_list = (
            StockDailyQuote.objects(code=item.code).only("date").order_by("+date")
        )
        if quote_list:
            ...
            if most_recent_quote_date == self.most_recent_quote_date:
                ...
                sliced_trade_calendar_set = set(
                    self.trade_calendar[start_date_index : end_date_index + 1]
                )
    ...
    def output(self):
        ratio_1 = (
            self.counter_dict["GOOD"]
            / (
                self.list_len
                - self.counter_dict["NO_DATA"]
                - self.counter_dict["OUT_OF_DATE"]
            )
        ) * 100
        print(
            f"Of all {self.list_len} {self.target_name} data, "
