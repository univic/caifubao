...
class MACrossSignalProcessor(SignalProcessor):
    ...
    def prepare_input(self):
        logger.info(
            f"Reading factor data for {self.stock_obj.code} - {self.stock_obj.name}"
        )
        # queryset
        pri_ma_factor_qs = FactorDataEntry.objects(
            stock=self.stock_obj, name=self.pri_ma
        )
        ref_ma_factor_qs = FactorDataEntry.objects(
            stock=self.stock_obj, name=self.ref_ma
        )
    ...
    def perform_calc(self, *args, **kwargs):
        self.process_df["pri_above_ref"] = (
            self.process_df[self.pri_ma] > self.process_df[self.ref_ma]
        )
        ...
        self.process_df["pri_up_cross_ref"] = (
            self.process_df["pri_above_ref"] & self.process_df["pri_cross_ref"]
        )
...
class PriceMARelationProcessor(SignalProcessor):
    ...
    def prepare_input(self):
        ...
        hfq_price_qs = StockDailyQuote.objects(stock=self.stock_obj, name=self.ref_ma)
        # convert queryset to json
        ref_ma_factor_query_json = ref_ma_factor_qs.as_pymongo()
        hfq_price_query_json = hfq_price_qs.as_pymongo()
        # convert json to df
        ref_ma_factor_df = pd.DataFrame(ref_ma_factor_query_json)
        hfq_price_df = pd.DataFrame(hfq_price_query_json)
        # set index
        ref_ma_factor_df.set_index("date", inplace=True)
        hfq_price_df.set_index("date", inplace=True)
        # rename column
        ref_ma_factor_df.rename(columns={"value": self.ref_ma}, inplace=True)
