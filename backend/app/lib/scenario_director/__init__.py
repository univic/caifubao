...
class ScenarioDirector(object):
    ...
    def update_dt(self, trade_calendar, backtest_current_datetime=None):
        ...
        self.real_world_prev_complete_trading_day = (
            trading_day_helper.determine_most_recent_previous_complete_trading_day(
                trade_calendar=trade_calendar, given_time=self.real_world_datetime
            )
        )
        if self.is_backtest:
            if backtest_current_datetime:
                ...
                self.current_datetime_prev_complete_trading_day = (
                    self.backtest_prev_complete_trading_day
                )
            else:
                logger.error("backtest time not filled")
        else:
