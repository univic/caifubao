...
class BasicBackTester(object):
    ...
    def before_run(self):
        ...
        self.backtest_name = (
            f"{self.strategy_name}-{self.start_date}-{self.end_date}-{dt_str}"
        )
    ...
    def main_sequence(self):
        ...
        for i, t in enumerate(self.backtest_periodic_task_list):
            ...
            self.scenario.update_dt(
                trade_calendar=self.trade_calendar, backtest_current_datetime=t
            )
    ...
    def get_backtest_periodic_task(self):
        ...
        self.backtest_periodic_task_list = [
            date for date in self.trade_calendar if date >= start_date
        ]
