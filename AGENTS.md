...
```bash
# Rebuild the venv only when it is missing or stale:
datahub/.tools/python312/bin/python3.12 -m venv datahub/.venv
datahub/.venv/bin/pip install -r datahub/requirements.txt pytest

# Tests and lint. Pass the CI rule set explicitly: CI pins a specific ruff
# (see RUFF_VERSION in .github/workflows/ci.yml) and relies on its default
# select, but a newer local ruff can default to more rules and then report
# hundreds of pre-existing findings that CI does not flag. `E4,E7,E9,F` is
# that CI default.
datahub/.venv/bin/python -m pytest datahub/app/test/
