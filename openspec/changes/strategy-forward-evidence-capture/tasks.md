...
- [x] 1.2 失败测试先行（红→绿）：同日按时运行 → FORWARD；SKIPPED→当日 FORWARD
...
- [x] 1.3 model：`evidence_kind` choices 增 FORWARD，default="REPLAY"（缺省即
      REPLAY）；新增 `strategy_forward_windows` 集合（model_version/horizon 上
      ACTIVE 唯一约束）

## 2. 实现

- [x] 2.1 runner FORWARD 分类：`decision_at < open(next_execution_date(signal_date,
...
- [x] 2.2 CLI：`forward certify`（开窗，append-only）、`forward progress`
      （count/120、首末日期、缺口、窗口状态）
- [x] 2.3 nav recompute 只改派生 NAV 字段（不改 target_holdings/status/evidence_kind/
      timestamps/config_hash）
- [x] 2.4 focused/full datahub pytest 全绿、Ruff、OpenSpec strict

## 3. Gates

- [x] 3.1 spec-guardian / contract-reviewer / qa-reviewer
