...
- [x] 1.4 镜像空头腿 `fwd_short_h{h}`/`blocked_short_h{h}`（`limit_down_entry`/
  `limit_up_exit`），多空价差不混用两侧样本
- [x] 1.5 单测：周末跨 h=1、T+1/h 偏移、停牌与涨跌停阻塞、board-aware 板别、
  开盘涨停/收盘涨停的区分、缺前收盘、缺行、镜像空头腿
...
- [x] 2.3 走查衰减改为带符号（对齐 `h20_excess_alpha._walk_forward_decay`）：样本外变号
  必须失败，而不是 decay=0 通过
- [x] 2.4 过滤不可交易改为按各自方向过滤（多头买不到 / 空头卖不出各自阻塞）；
  `turnover_report` 按 horizon 节奏（每 h 个 session 比较一次）；IC 增加 Newey-West
  t 统计量（lag = h−1）
...
- [ ] 3.3 若未来要统一 4 套实现，另开迁移 change 并重跑受影响的研究结论
- [ ] 3.4 组合层语义（整手、最低佣金、市场冲击、排队）留给 `strategy-paper-*`/`backtest`；
  本 lab 的价差仍是「统计价差」，不等于可部署策略
...
- [x] 4.3 qa-reviewer 的一轮 P1/P2 修正后重跑 `pytest datahub/app/test/`、`ruff`、
