...
1. **短线（h1/h5）在多空口径下不可交易**：毛价差 ≤0.43%，两腿成本 0.70%，净价差
   全为负。`gap_1` 是唯一两个 horizon 都过闸门的因子，但其净价差 −0.53%/−0.35%。
2. **h20 的负 IC 仍是唯一稳健的「已知答案」**：动量/趋势族 IC 全部为负、量级与审计
   一致；但按重叠标签修正后的 Newey-West t，`|t| ≥ 3.3`（Bonferroni）只剩下
   `volume_zscore_60`（−3.63）与 `trend_60`（−3.30），`momentum_20`（−3.04）、
   `rsi_14`（−2.83）不再达标。也就是说**「反向」是真的，但显著性被首版高估**。
...
2. **未做多重检验校正**：本次跑了 17 因子 × 3 horizon = 51 次检验，5% 阈值下 Bonferroni
   约需 |t| ≥ 3.3（**按 Newey-West t**）：只有 `volume_zscore_60`（−3.63）与
   `trend_60`（−3.30）能过；`momentum_20`（−3.04）、`rsi_14`（−2.83）只是接近。
   下一片必须先预注册假设再做校正。
