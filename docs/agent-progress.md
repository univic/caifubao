...
### 2026-09-08 09:22 CST — H20 快照审计与收益口径澄清

- 状态：进行中（工具与证据完成，等待最终审查和 PR CI）。
- 已完成：读取 #203–#208 交接；#203–#207 已合并，#208 为本次审计的开放交接来源。新增只读审计 CLI 与 [报告](autoresearch/runs/h20-excess-alpha/snapshot-audit-2026-09-08.md)。3525955 个主快照键与合并快照在所查九列一致；入/出场标签自连接零价格差异；45 条 dev 样本开收盘一致。缺失覆盖单独报告，不宣称全量数据正确。
- 已完成：复现 −1.26635%，确认是 nominal H20 持有期 cohort 均值，不能作为日收益复利为“五个月 −75%”。未修改 evaluator、候选、快照、官方 ledger，未开启 forward 窗口。
- 验证：新增 7 项测试；datahub 全量 592 passed；CI 固定 Ruff 0.15.15 全量通过；OpenSpec strict 16 passed。
- 下一步：完成本审计 PR；另起变更统一组合日净值与 research/dev 成交、持有期、费用和基准口径，再评估策略。
- 阻塞：无；行情缺失原因和全量独立源验证不属于本次已证实范围。
