...
| ID | 严重度 | 位置 | 问题 | 修复 |
|---|---|---|---|---|
| C1 | **P0** | scoring_service.py:184-210（每股 5 查）+ :718-726（信号衰减回看）+ components.py:353/365-374（行业×2）+ :575-579（CSI300）+ :646-657（逐文档 save） | **主路径逐股 N+1**：每股票×horizon ~8 读 + 1 写 → 15,000 次 `score_single_stock`/天 ≈ **18–20 万往返/天**（本地 3–5 分钟，500m CPU pod 下 ~6 分钟）；年度 replay ≈ 5,000 万往返、十几小时起。CPU 主因是每查询的 mongoengine Document 水合 | **已实现，待 dev 实测**：`score_all_stocks`/`score_all_stocks_ranked` 改按天预取——当日 quote/factor/signal/existing 各 1 查、`code__in` 全市场历史窗口 1 查（按**交易日** `max(minimum_quote_count, breakout_lookback, risk_lookback)+10` 取窗；窗口内行数不足的稀疏/停牌股回退逐股精确查询）、信号衰减窗 1 查、CSI300 1 查、行业分类+L1 指标各 1 查；行以 `as_pymongo()` 原始 dict + 字段默认骨架装配为 `_Row`（免 Document 水合），窗口分块（5 万行/块，concat 后释放 chunk 帧）建帧后 `groupby("code").indices` 按需物化——702k 行 × 8 投影字段合成实测 tracemalloc 峰值 ~102MB、保留 ~57MB（未分块单次建帧 ~420–460MB；QA 独立复测分块收益 ~2.3–3.7×，1Gi pod 约束）；窗口/衰减窗/行业按分量实读字段投影，existing 仅投影 `stock_code/horizon/status`（新增字段读取须同步白名单，harness 已实际拦下 `buy_count` 漏投影）。CSI300 用 `index_quotes_for(code,…)` 按该股自身 `[min(quote dates), d]` 过滤下发，稀疏股回退到窗口之前时各自读一次自身区间，避免评估日被静默降级为 self-proxy。持久化改 `bulk_write(UpdateOne({stock_code,date,horizon,model_version},{$set:…+updated_at,$setOnInsert:{generated_at}},upsert=True), ordered=False)`，与逐文档 save 语义一致（BLOCKED 行清 rank/percentile）；ranked 收尾在 bulk 写后读回落库 cohort 一次再排名。`DATAHUB_SCORING_BATCH=0` 回退逐股读/写路径。等价性：`datahub/app/test/test_scoring_batch_equivalence.py` 对同一天全市场逐字段 diff 为空（raw/ranked、全/单 horizon、replace=False 修复、dry_run、预取失败回退），含反向对照（人为扰动即报差异）与「读次数不随 cohort 规模增长」断言——harness 已实际拦下 pandas NaN 泄漏与稀疏股 CSI300 两处分歧。预取粒度已合并：`scoring_runner.run_scoring` 与 `ScoreReplayService.backfill_predictions` 每个日期只调用一次 `score_all_stocks`（`horizon=None` 覆盖全部 horizon），完整交易日由 3 份预取降为 1 份；`get_t_plus_n_day` 改缓存排序日历 + `bisect`（同一日历 15,600 次调用 54.0s → 0.011s，约 54s CPU/天）。**实测 before/after（2026-09-11 全市场，`--dry-run`）**：
dev（5,561 股，旧镜像 `sha-e92bd93` 逐股路径 → 新镜像 `sha-c2dd5b34`）：h20 单 horizon **80.4 s → 21.8 s（3.7×）**、全部 horizon **247.1 s → 49.0 s（5.0×）**（16,683 条预测）；
research（5,562 股）：h20 **87.0 s → 12.6 s（6.9×）**、全部 horizon **261.9 s → 31.4 s（8.3×）**。
**真实库全市场等价性（`score equivalence-check --apply`，含持久化与收尾，逐 horizon 两遍）**：dev 0 diff / 16,683 行（h5 39.8→16.9 s、h20 66.6→29.2 s、h60 102.4→46.4 s）；research 0 diff / 16,686 行（h5 41.9→14.2 s、h20 65.8→20.1 s、h60 106.1→27.8 s）。写路径整体加速幅度（2.3–3.4×）低于 dry-run（5.0–8.3×），因为收尾阶段（完整性校验/rank/推荐/行业聚合）不在 C1 范围内。该检查在 dev 上**实际拦下一处分歧**：信号读取顺序（`stock_code=X` vs `stock_code__in`）导致 5,561 行中 27 行 `explanation` 信号数组顺序不同 —— 已由 #229 修复（两条读取路径按 `signal_name` 定序），fake harness 无法发现。**内存实测（2026-09-13）**：单份全市场窗口（674k 行）RSS 峰值 **~1.06GiB**（h20 单 horizon 364k 行 ~683MiB）——tracemalloc 的 ~102MB 只覆盖 Python 分配，严重低估；增量来自 pymongo 解码 674k 条 BSON 的分配器高水位（最终列式帧 deep 内存仅 ~82MB）。因此 dev datahub 的 1Gi limit 被 OOMKill（exit 137，实测），生产 scoring CronJob 的 2Gi 可用但余量约 2×；dev overlay 已提到 2Gi 过渡。根治 = 按 ~900–1,000 code 分组预取（实测 RSS 峰值 ~294MiB），见 tasks 3.1b（未实现）。spec scenario 9（验证增量门槛，C6/2.7 遗留）不在 C1 范围。dev 真实库全市场同日逐字段比对与 ranked before/after 仍待采集（tasks 3.5a/5.3） |
...
| C4 | **P0** | factor_eval.py:104-106（base 行情逐观测）、:117-125（每 horizon 未来行情）、:306-315（`_compute_decay` 用 6 个 horizon 重跑整个 `_build_dataset`）；调用方 tech_factor_runner.py:133-135 已把全市场行情载入内存 | 逐观测点 N+1：全市场年评估 1.25M 观测 ×(1+3) + decay 1.25M×(1+6) ≈ **1,375 万次查询**（1ms 也要 ~4 小时），而行情已在内存 | `evaluate` 增加可选 `quote_frame`（code→按日排序 close 表）；前向收益 `groupby("code")["close_hfq"].shift(-h)/close - 1` 向量化；decay 复用同一数据集只补 1/3/10 三个 horizon 的 shift。DB 查询归零 | **已实现（task 3.19/3.20，2026-09-13）**：`evaluate`/`_build_dataset` 接受预载 `quote_frame`，前向收益改 `groupby("code").shift(-h)` + 显式 `int(h*1.5)` 日历窗；decay 复用同一数据集；`tech_factor_runner` 改 `.only()` + `as_pymongo()` + `_Row` 装配（不再逐行水合 Document）；并修掉该 CLI 硬编码 `MONGO_URI`(localhost/caifubao) 导致**任何已部署环境都连不上**的缺陷。**实测（dev，`gap_ratio` 2026-06-01…06-10，40,256 观测，h20）**：434.4 s → **49.9 s（8.7×）**，逐观测往返 362,304 → **0**，且 `observation_count`/`ic`/`icir`/`decay`/`quintiles` 逐字段一致；全市场一年外推 ~3.2 h → ~25 min（余下为因子逐股计算与建帧）。frame 路径内存 ~540B/行（200k 行峰值 ~107MB），全市场一年需分段评估。
| C5 | P1 | scoring_service.py:257-270（assign_ranks 逐文档 save，1.5 万次/天×delta `$set`）、replay_service.py:38-66（每 horizon 重新拉股票全集 + 3 次重读刚写完的当日结果 :140-146/:258-265/:279-286）、components.py:353/365-374/575-579（行业分类、行业指标、CSI300 可按天缓存，各省 1.5 万次/天）、scoring_service.py:67-69（`get_t_plus_n_day` 每次排序日历） | 高频重复开销合计每晚数万往返 + 分钟级纯 CPU | **已实现，dev 实测**：`assign_ranks` 仅为 rank/percentile 实际变化行生成 `UpdateOne`，空 delta 零 bulk write（2.5）；ranked 收尾消费内存结果——`assign_ranks(predictions=)` 免 cohort 重查并同步改写内存对象、`_require_complete_prediction_set(persisted_codes=)` 内存完整性校验（含 replace=False 修复路径存留 BLOCKED 行计数）、BLOCKED 行不入内存排名、`_upgrade_recommendations(predictions=)` 免收尾 cohort 重读（2.6 及后续 PR）。**实测**（dev 2 核、#197 代码、单日全市场 ranked h20 replace 重算 5,188 股）：**87 s**（同负载旧代码 0.3 核 ≈600 s、4 核 95 s——新代码 2 核已快于旧代码 4 核）。遗留：行业/指数缓存与日历优化（后续 C1/C5 工作） |
| C6 | P1 | verification_service.py:32-48、:59-65、:74/:104；入口 scoring_runner.py:427-434（from_date=None） | 验证逐条「1 查未来行情 + 1 save」。*量级修正：查询含 `target_date__lte=today`，在途 TRACKING（target 未到）被挡在外面，稳态日处理量是「当日到期未验」~1.5 万条（≈3 万往返），并非深审初报的 42.5 万条/85 万往返；但 verify 中断、停牌积压、`--replace` 重置后池子会放大* | **部分已实现，待 dev 实测**：新增批量路径 `verify_predictions_batch`/`_verify_many`——未来行情按 stock_code 单次拉取（`.only()` 投影含 hfq 价格字段）后按 (date, target_date] 双界 bisect 切片、逐预测判定，status/verification 更新走 `bulk_write($set)`；`verify_predictions` 路由到批量路径（2.7）；候选查询加 `.only()` 投影（仅验证所需字段，免 ~1.5 万条/日全量水合，后续 PR）。遗留：「仅验到期未验且自上次验证后有新行情」增量门槛（`last_verified_date` 或分桶）未实现 |
