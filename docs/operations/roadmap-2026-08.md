...
> **历史状态注记（2026-09-16）**：本文写作于两轴环境模型（`docs/architecture/
> environment-model.md`）确立之前。文中的「prod」「prod-dev 同步」指当时的稳定
> 运行环境（现称**旧 stable / retired production**，迁移期数据权威环境），不代表
> 实盘 production——`prod`（trading/production）目标态尚未启用。本文为历史记录，
> 部分待办仍开放；现行环境口径以环境模型文档为准。
